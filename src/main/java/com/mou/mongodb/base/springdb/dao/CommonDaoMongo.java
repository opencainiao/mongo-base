...
public class CommonDaoMongo implements IBaseDaoMongo {
	...
	public <T> List<T> findBatch(Query query, Class<T> entityClass, Sort sort,
			String collectionName) {
		return FindBatchUtil
				.findBatch(query, entityClass, sort, collectionName);
	}

	public <T> List<T> findBatchPart(Query query, DBObject returnFields,
			Sort sort, Class<T> entityClass) {
		return FindBatchUtil.findBatchPart(query, returnFields, sort,
				entityClass);
	}
	...
	public <T> List<T> findBatchPage(Query query, Class<T> entityClass,
		...
		return FindBatchUtil.findBatchPage(query, entityClass, collectionName,
				pageVO, sort);
	}

	public <T> List<T> findBatchPartPage(Query query, DBObject returnFields,
			Class<T> entityClass, PageVO pageVO, Sort sort) {
		return FindBatchUtil.findBatchPartPage(query, returnFields,
				entityClass, pageVO, sort);
	}
	...
	public <T> List<T> findBatchPartPage(Query query, DBObject returnFields,
			...
			Sort sort) {
		return FindBatchUtil.findBatchPartPage(query, returnFields,
				entityClass, collectionName, pageVO, sort);
	}
	...
	public <T> DBObject updateOneById(String _id, DBObject returnFields,
		...
		return UpdateUtil.updateOneById(_id, returnFields, update,
				collectionName);
	}
	...
	public <T> DBObject updateOneByCondition(DBObject query,
			...
			Class<T> entityClass) {
		return UpdateUtil.updateOneByCondition(query, returnFields, update,
				upsert, entityClass);
	}
